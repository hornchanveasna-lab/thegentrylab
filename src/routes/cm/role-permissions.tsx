...
function CMRolePermissionsPage() {
  ...
  const [saving, setSaving] = useState<CMPermissionAction | null>(null);

  const invalidate = () => queryClient.invalidateQueries({ queryKey: ["cm_role_permissions", user?.id] });
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0b] text-white font-sans">
      <main className="max-w-md sm:max-w-xl md:max-w-3xl lg:max-w-5xl mx-auto w-full px-4 pt-6 pb-10">
        ...
        <div className="mb-4">
          <FieldSelect value={jobRole} onChange={setJobRole} searchable
            options={CM_JOB_ROLES.map((r) => ({ value: r, label: t(`team.jobRole.${r}`) }))} />
        </div>

        <Card title={t(`team.jobRole.${jobRole}`)}>
                  ...
                  {ACTIONS.map(({ action }) => {
                    ...
                    return (
                      <div key={action} className="flex items-center justify-center">
                        <input type="checkbox" checked={checked} disabled={saving === action}
