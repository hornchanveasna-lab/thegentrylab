...
/** Groups items into per-day, per-project sections (newest day first) so a
 *  project's punch items raised on the same day read as one combined
 *  group instead of a flat list — each item's `doc_number` already carries
 *  that same day, since it's generated by the day-scoped RPC. */
function groupTasksByDay<T extends CMTask>(items: T[]): { day: string; projectId: string; items: T[] }[] {
  const map = new Map<string, T[]>();
  for (const item of items) {
    const key = `${item.project_id}|${item.created_at.slice(0, 10)}`;
  ...
  return Array.from(map.entries())
    ...
    .map(([key, groupItems]) => {
      ...
      return { day, projectId, items: groupItems };
    });
}
...
function CMPunchListPage() {
  ...
  const openGroups = useMemo(() => groupTasksByDay(open), [open]);
  const doneGroups = useMemo(() => groupTasksByDay(done), [done]);
            ...
            <>
                ...
                <div className="flex flex-col gap-5">
                  {openGroups.map((group) => (
                    <div key={`${group.projectId}|${group.day}`} className="flex flex-col gap-2">
                      <div className="flex items-center gap-2 px-1">
                        <span className="font-mono text-[10px] uppercase tracking-widest text-white/35">{formatDayHeader(group.day, lang)}</span>
                        <span className="font-mono text-[9px] text-white/20">{group.items.length}</span>
                      </div>
                      <div className="flex flex-col gap-2">
                        {group.items.map((t) => <PunchItemCard key={t.id} item={t} showDate={false} projectName={viewAll ? (t as CMTaskWithProject).projectName : undefined} />)}
                      </div>
                    </div>
                  ))}
                </div>
