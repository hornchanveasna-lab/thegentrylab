    ...
    <div className="min-h-screen bg-[#0a0a0b] text-white font-sans print:bg-white print:text-black">
      <main className="max-w-md mx-auto w-full px-4 pt-6 pb-16">
        <div className="flex items-center gap-3 mb-6 print:hidden">
          ...
          <h1 className="text-xl font-extrabold tracking-tight text-white">Report</h1>
        </div>
        ...
        <div className="flex flex-col gap-3 mb-6 print:hidden">
          ...
          <div className="flex gap-2">
            {[{ label: "7 days", days: 7 }, { label: "30 days", days: 30 }, { label: "90 days", days: 90 }].map((r) => (
        ...
        {!projectId && <p className="text-white/30 text-sm print:hidden">Pick a project and date range to generate a report.</p>}
          ...
          <div className="flex flex-col gap-4">
            ...
            {!logsLoading && (
              <div className="grid grid-cols-3 gap-3 print:border print:border-black/10 print:rounded-none">
                {[
                  { label: "Diary entries", value: filtered.length },
                  { label: "Avg workforce", value: avgWorkforce ?? "—" },
                  { label: "Latest progress", value: latestProgress != null ? `${latestProgress}%` : "—" },
                ].map((s) => (
            ...
            {!logsLoading && totalPhotos > 0 && <p className="text-[12px] text-white/40 print:text-black/60">{totalPhotos} photo{totalPhotos === 1 ? "" : "s"} logged in this range.</p>}

            {!logsLoading && filtered.length === 0 && <p className="text-white/30 text-sm">No diary entries in this date range.</p>}
            ...
            <div className="flex flex-col gap-2">
              {filtered.map((l) => (
                <div key={l.id} className="rounded-2xl bg-[#0d0d0e] print:bg-white print:border print:border-black/10 px-4 py-3">
                  ...
                  {l.weather && <p className="font-mono text-[9px] uppercase tracking-widest text-white/30 print:text-black/50 mb-1">{l.weather}{l.workforce_count != null ? ` · ${l.workforce_count} workers` : ""}</p>}
                  {l.activities && <p className="text-[12px] text-white/60 print:text-black/80">{l.activities}</p>}
                  {l.issues && <p className="text-[11px] text-red-400/80 print:text-red-700 mt-1">Issue: {l.issues}</p>}
                </div>
              ))}
            </div>
          </div>
