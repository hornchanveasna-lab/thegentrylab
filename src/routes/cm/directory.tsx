...
function NewContactSheet({ ownerId, projectId, onClose, onCreated }: { ownerId: string; projectId: string | undefined; onClose: () => void; onCreated: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center bg-black/70" onClick={onClose}>
      <div className="w-full sm:max-w-lg bg-surface-1 rounded-t-3xl sm:rounded-3xl max-h-[85vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
        <div className="w-10 h-1 rounded-full bg-surface-3 mx-auto mt-3 sm:hidden" />
        <div className="flex items-center justify-between px-6 pt-4 pb-2">
          <h2 className="font-extrabold text-base tracking-tight text-text-primary">{t("directory.new")}</h2>
          <button onClick={onClose} className="w-8 h-8 rounded-full flex items-center justify-center bg-surface-2 hover:bg-surface-3 text-text-muted hover:text-text-primary transition-colors">×</button>
        </div>
...
function ContactCard({ contact, ownerId, onChanged, linked }: { contact: CMDirectoryContact; ownerId: string; onChanged: () => void; linked?: CMLinkedMember }) {
  ...
  return (
    <div className="rounded-2xl bg-surface-1 px-5 py-4 flex items-start justify-between gap-3">
      <div className="min-w-0 flex items-center gap-3">
        <label className="relative shrink-0 cursor-pointer">
          ...
          {uploading && <span className="absolute inset-0 rounded-full bg-black/50 flex items-center justify-center text-text-muted text-[9px]">…</span>}
        </label>
        <div className="min-w-0">
          ...
          {(contact.phone || contact.email) && (
            <p className="font-mono text-[10px] text-text-subtle mt-1">{[contact.phone, contact.email].filter(Boolean).join(" · ")}</p>
          )}
          {contact.notes && <p className="text-[11px] text-text-subtle mt-1.5">{contact.notes}</p>}
        </div>
      </div>
    ...
    <div className="min-h-screen bg-background text-text-primary font-sans">
      <main className="max-w-md sm:max-w-xl md:max-w-3xl lg:max-w-5xl mx-auto w-full px-4 pt-6 pb-28">
        <div className="flex items-center gap-3 mb-6">
          ...
          <h1 className="text-xl font-extrabold tracking-tight text-text-primary">{t("directory.title")}</h1>
        </div>
